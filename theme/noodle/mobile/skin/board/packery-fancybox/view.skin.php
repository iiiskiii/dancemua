...
;
    ...
    <section id="bo_v_atc">
        ...
        <div id="bo_v_share">
	        ...
	        <?php if ($scrap_href) { ?><a href="<?php echo $scrap_href;  ?>" target="_blank" class="btn btn-light" onclick="win_scrap(this.href); return false;">스크랩</a><?php } ?>
	    </div>
<!----masonry 추가로 변경 시작---->
	<?php
		// 파일 출력
		...
		if($v_img_count) {
			...
			echo "<div class='grid'>\n";// 이미지 여백 때문에 id=\"bo_v_img\" 제거함
				foreach($view['file'] as $view_file) {
					if( !trim($view_file[href]) ) continue;  //데이터가 없는것은 패스하고 그다음 로직으로 이동
