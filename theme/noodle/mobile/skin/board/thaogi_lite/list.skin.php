...
<!-- 게시판 목록 시작 { -->
<div class="fz_wrap" style="width:100%" >
    <!-- 게시판 카테고리 시작 { -->
    <?php if ($is_category) { ?>
    <nav id="bo_cate">
...
<div style="width:90%;margin:0 auto;">
	<div style="width:100%;overflow: hidden;">
		<?php if ($is_checkbox) { ?>
		<div id="gall_allchk" style="float: left;">
		...
		<div class="fr">
			...
			<?php if ($admin_href&&$member['mb_id']=="iiiskiii79") { ?><li style="list-style: none;display:inline-block;"><a href="<?php echo $admin_href ?>" class="btn_admin">관리자</a></li><?php } ?>
            <?php if ($write_href) { ?><li style="list-style: none;display:inline-block;"><a href="<?php echo $write_href ?>" class="btn_b02">글쓰기</a></li><?php } ?>
            <!-- <?php if ($admin_href&&$member['mb_id']=="iiiskiii79") { ?><a href="<?php echo $admin_href ?>" class="list_btn btn_adm">관리자</a><?php } ?>
	...
	<ul class="fz_gallery_list">
		...
		for ($i=0; $i<count($list); $i++) {
		 ...
		 ?>
		<li class="col-sm-12 col-xs-6 <?=$wr_id == $list[$i]['wr_id'] ? " active" : ''?>">
			<a href='<?php echo $list[$i]['href'];?>' class="fz_gallery_li_wrap">
				<span class="fz_gallery_thumb"><!-- <?php echo $img_content;?> -->
				...
				<span class="fz_gallery_box">
					...
					<span class="fz_gallery_content"><?=cut_str(str_replace("&nbsp;", "", trim(strip_tags($list[$i]['wr_content']))), 200)?></span>
	...
	<div class="fz_footer">
		...
		<div class="fr">
            <!-- <?php if ($list_href) { ?><a href="<?php echo $list_href ?>" class="list_btn btn_list">목록</a><?php } ?> -->
			<?php if ($admin_href) { ?><li style="list-style: none;display:inline-block;"><a href="<?php echo $admin_href ?>" class="btn_admin" >관리자</a></li><?php } ?>
            <?php if ($write_href) { ?><li style="list-style: none;display:inline-block;"><a href="<?php echo $write_href ?>" class="btn_b02">글쓰기</a></li><?php } ?>
            <!-- <?php if ($admin_href&&$member['mb_id']=="iiiskiii79") { ?><a href="<?php echo $admin_href ?>" class="list_btn btn_adm">관리자</a><?php } ?>
            <?php if ($write_href) { ?><a class="list_btn btn_write" href="<?=$write_href?>" title="글쓰기">글쓰기</a><?php } ?> -->
		</div>
